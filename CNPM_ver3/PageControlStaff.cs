...
namespace CNPM_ver3
{
    ...
    {
        // Initialize variables used in form
        ...
        // Initialize data and UI when loading Page
        private void PageControlStaff_Load(object sender, EventArgs e)
        {
            ...
            //FE
            AsT_dataGridView_subtasks.ReadOnly = false;
        ...
        // Clear request current in field
        ...
        // Open current task when double click
        ...
        private void AsT_loadTaskInfo()
        {
            ...
            if (subTasks != null)
            {
                DataColumn newColumn = new DataColumn("Đã xong", typeof(bool));
                subTasks.Columns.Add(newColumn);
                foreach (DataRow row in subTasks.Rows)
                {
                    //MessageBox.Show("DOne : " + row["SJ_DONE"]);
                ...
                subTasks.Columns["SJ_NAME"].ColumnName = "Nhiệm vụ con";
                ...
                AsT_dataGridView_subtasks.Columns["SJ_DATE"].Visible = false;

                AsT_dataGridView_subtasks.Columns["Đã xong"].ReadOnly = true;
            }
        ...
        private void AsT_button_assign_Click(object sender, EventArgs e)
        {
            ...
            if (task.SubmitTask(taskID, Users.PK, subTaskIDs, files))
            {
                AsT_btn_close_Click(sender,e);
                MessageBox.Show("Nộp nhiệm vụ thành công");
            }
            else
            {
                MessageBox.Show("Nộp nhiệm vụ không thành công");
            }
        }
        ...
        // Number of current files in task
        ...
        // Delete the lastest files , number of task decreace by 1
        ...
        // Download resource file when double click
        ...
        // Download file when click to cell in datagridview
        ...
        // Load page notification receiver
        ...
        // Open notification when click to cell in datagridview
        ...
        // Open notification when click icon bell
        ...
        // See the task has been asign when click to cell
        private void AsT_dataGridView_subtasks_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            // Check if the clicked cell is a checkbox cell
            if (e.ColumnIndex == 4 && e.RowIndex >= 0)
            {
                DataGridViewCheckBoxCell checkboxCell = (DataGridViewCheckBoxCell)AsT_dataGridView_subtasks.Rows[e.RowIndex].Cells[e.ColumnIndex];
                if ((bool)checkboxCell.Value)
                {
                    AsT_dataGridView_subtasks.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = false;
                }
                else
                {
                    AsT_dataGridView_subtasks.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = true;
                }
                task.SubmitOneSubtask(AsT_dataGridView_subtasks.Rows[e.RowIndex].Cells["SJ_ID"].Value.ToString());
            }
        }
    }
}
